...
class BusinessRegistrationControllerSpec extends PlaySpec with GuiceOneServerPerSuite with MockitoSugar {
  ...
  "BusinessRegistrationController" must {
    ...
    "register" must {
      val successResponse =
        Json.parse( """{"processingDate":"2015-12-17T09:30:47Z","sapNumber":"sapNumber","safeId":"XE000123456789","agentReferenceNumber":"01234567890"}""")
      ...
      "respond with OK" in new Setup {
        val inputJsonForNUK: JsValue =
          Json.parse(
          """{"acknowledgementReference":"session-ea091388-d834-4b34-8b8a-caa396e2636a",
              |"organisation":{"organisationName":"ACME"},"address":{"addressLine1":"111","addressLine2":"ABC Street","addressLine3":"ABC city","addressLine4":"ABC 123","countryCode":"UK"},"isAnAgent":false,"isAGroup":false,"nonUKIdentification":
              |{"idNumber":"id1","issuingInstitution":"HMRC","issuingCountryCode":"UK"}}"""
            .stripMargin)
